...
=========================
MongoDB persistence layer for BingeFinder's series catalog.

Design notes:
    - A single `pymongo.MongoClient` is reused per `MongoDBManager`
      instance (PyMongo pools connections internally).
    - The `series` collection is uniquely indexed on `series_id` so
...
class MongoDBManager:
    ...
    def ensure_indexes(self) -> None:
        """
        Ensure a unique index on `series_id` exists — idempotent, so
        ...
        `series_id_1` from a previous default `create_index("series_id")`
        call, vs. this method's `name="uniq_series_id"`) makes MongoDB
        ...
        is inspected first:
          ...
          1. No index on `series_id` at all -> create the unique index.
          2. An existing index on `series_id` is already unique -> reuse
             it as-is (whatever it's named); nothing to do.
          3. An existing index on `series_id` is NOT unique -> check the
             collection for duplicate `series_id` values first:
        ...
        Raises:
            MongoDatabaseError: If duplicate `series_id` values are found
        ...
        # Find any existing index whose key spec is exactly {series_id: 1}
        # (a single-field ascending index), regardless of its name.
        existing_series_id_index = None
        for index_name, index_spec in existing_indexes.items():
            if index_spec.get("key") == [("series_id", 1)]:
                existing_series_id_index = (index_name, index_spec)
                break
        ...
        logger.warning(
            "Existing index '%s' on 'series_id' is not unique; checking for duplicate "
            "series_id values before attempting to replace it.",
            index_name,
        )
        duplicates = self._find_duplicate_series_ids()
        if duplicates:
            raise MongoDatabaseError(
                "Cannot create a unique index on 'series_id': duplicate values found "
        ...
        logger.info(
            "No duplicate series_id values found; dropping non-unique index '%s' and "
    ...
    def _create_unique_series_id_index(self) -> None:
        ...
        try:
            self.series.create_index("series_id", unique=True, name="uniq_series_id")
        except PyMongoError as exc:
            logger.error("Failed to create unique index on series_id: %s", exc)
            raise MongoDatabaseError(f"Failed to create unique index on series_id: {exc}") from exc

    def _find_duplicate_series_ids(self) -> List[Any]:
        """
        Return the list of `series_id` values that appear on more than
        ...
        try:
            ...
            logger.error("Failed to check for duplicate series_id values: %s", exc)
            raise MongoDatabaseError(f"Failed to check for duplicate series_id values: {exc}") from exc

    def upsert_series(self, series_document: Dict[str, Any]) -> str:
        """
        Insert a new series document, or update the existing one matched
        by `series_id`, avoiding duplicates.

        Args:
            series_document: Dict with at least a "series_id" key, plus
        ...
        Returns:
            ...
            existing document (matched by series_id) was written to.

        Raises:
            ValueError: If `series_document` has no "series_id".
            MongoDatabaseError: If the write fails.
        """
        series_id = series_document.get("series_id")
        ...
        try:
            ...
            logger.error("Failed to upsert series series_id=%s: %s", series_id, exc)
            raise MongoDatabaseError(f"Failed to upsert series series_id={series_id}: {exc}") from exc
    ...
    def get_series_by_id(self, series_id: int) -> Optional[Dict[str, Any]]:
        """
        Fetch a single series document by its `series_id`.
        ...
        try:
            ...
            logger.error("Failed to fetch series series_id=%s: %s", series_id, exc)
            raise MongoDatabaseError(f"Failed to fetch series series_id={series_id}: {exc}") from exc
